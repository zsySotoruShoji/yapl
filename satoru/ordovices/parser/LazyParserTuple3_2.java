...
public class LazyParserTuple3_2<A1,A2,A3> implements Parser<Tuple3<A1,A2,A3>>{

    private final Parser<A1> p1;
    private final Supplier<Parser<A2>> p2;
    private final Parser<A3> p3;

    public LazyParserTuple3_2(
            Parser<A1> p1, Supplier<Parser<A2>> p2, Parser<A3> p3){
        this.p1 = p1;
        this.p2 = p2;
        this.p3 = p3;
    }

    public static <A1,A2,A3> LazyParserTuple3_2<A1,A2,A3> t(
            Parser<A1> p1,
            Supplier<Parser<A2>> p2,
            Parser<A3> p3){
        return new LazyParserTuple3_2<>(p1, p2, p3);
    }
    ...
    public Tuple3<A1,A2,A3> primitiveParse(TokenList tl) {
        ...
        if(tl.isFailed()) return null;
        return new Tuple3<>(a1, a2, p3.primitiveParse(tl));
    }
    ...
    public <R> Parser<R> primitiveApply(Function3<A1,A2,A3,R> f){
        return tl -> {
            ...
            return f.primitiveApply(a1, a2, p3.parse(tl));
        };
    }
    ...
    public <R> Parser<R> apply(Function3<A1,A2,A3,R> f){
        return tl -> {
            ...
            if(tl.isFailed()) return null;
            A2 a2 = p2.get().parse(tl);
            return f.apply(a1, a2, p3.parse(tl));
        };
    }
